...
# CASO AINDA NAO TIVER LOGADO MOSTRA O PAINEL
if not st.session_state.logado:
    # logo titan
    st.markdown(
        """
        <div style='text-align:center;'>
            <img src='https://raw.githubusercontent.com/AGTTITAN99/titandashboard/refs/heads/main/identidade-titan%203.png' width='220'>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    st.markdown(
        f"""
        <h1 style='text-align:center; color:{text_color}; font-size:40px; font-weight:700;'>
        🔒 Acesso Restrito
        </h1>
        """,
        unsafe_allow_html=True
    ...
    st.markdown(
        ...
        Digite a senha para acessar o Dashboard Titan:
        </p>
        """,
        unsafe_allow_html=True
    )
...
try:
    ...
    # se nao encontrar, assume modo claro
    theme_base = "light"

# CORES NOVAS
if theme_base == "light":
    # BRANCO
    background_color = "#121212"  # fundo escuro
...
if theme_base == "light":
    # força o texto branco para modo claro
    text_color = "#FFFFFF"
else:
    text_color = "#FFFFFF"

# calculo para pegar as metricas
...
# exibição estilizada
def metric_label(titulo, valor):
    st.markdown(
        f"""
        <div style="margin-bottom:12px;">
            <div style="font-size:16px; color:{text_color}; font-weight:500;">{titulo}</div>
            <div style="font-size:32px; color:{text_color}; font-weight:700;">{valor}</div>
        </div>
        """,
        unsafe_allow_html=True
    )

metric_label("Faturamento Total", f"${faturamento_total:,.0f}")
metric_label("Custo Total", f"${custo_total:,.0f}")
metric_label("Profit Total", f"${profit_total:,.0f}")
metric_label("ROI Total", f"{roi_total:.2f}%")
metric_label("Ticket Médio (Profit)", f"${ticket_medio_profit:,.0f}")
metric_label("Ticket Médio (ROI)", f"{ticket_medio_roi:.2f}%")
